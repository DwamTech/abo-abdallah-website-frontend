...
test("home and index cards expose only a safe optional preview URL", () => {
  ...
  const previewCard = scientificVideoCardSchema.parse({
    ...card,
    preview_url: "https://cdn.example.test/preview.mp4",
  });
  assert.equal(previewCard.preview_url, "https://cdn.example.test/preview.mp4");
});
