...
test("the public list page loads both index types and keeps existing registries", () => {
  ...
  assert.match(page, /"subject_index"/);
  assert.match(page, /"alpha_index"/);
...
test("the detail route is dynamic, validates the number, and has no static mock fallback", () => {
  ...
  assert.doesNotMatch(envExample, /PUBLIC_LIBRARY_SUBJECT_INDEXES_ENABLED/);
});
