...
test("all six public module contracts accept views_count with a zero fallback", () => {
  for (const file of contracts) {
    const content = source(file);
    assert.match(content, /views_count:\s*viewCount/, file);
    assert.match(content, /const viewCount = [^;]*\.catch\(0\)/, file);
  }
});
...
test("every active home and index card family renders the view metric", () => {
  for (const file of [...homeSections, ...indexes]) {
    const content = source(file);
    assert.match(content, /<ViewCount/, file);
...
test("detail pages show and record views only through same-origin BFF routes", () => {
  for (const file of details) {
    const content = source(file);
    assert.match(content, /TrackedViewCount|ViewCount/, file);
    assert.match(content, /\.views_count/, file);
  }
